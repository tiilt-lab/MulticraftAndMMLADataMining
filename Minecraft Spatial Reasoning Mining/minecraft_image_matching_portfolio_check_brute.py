...
images_files= ["portfolio_image_2.jpg"] #store the filename for each image
...
for line in csv_file:
	...
	#time_step = 0.2
	'''
	file_name = "portfolio_image_2_33.png"
	...
	frame_counter = 0
	portfolio_frames = 0
	portfolio_frame_start = 0
	portfolio_frame_end = 0
	found_portfolio = False
	#v_file_name = "_".join(os.path.split(video_file))
	...
	while(cap.isOpened()):
		...
		if ret == True: # and image_index<len(images_files):
			...
			if min_val <= 0.02:
				portfolio_frames+=1
				if found_portfolio == False:
					found_portfolio = True
					portfolio_frame_start = frame_counter
					name = "_".join(["portfolio",str(video_index),video_file.replace("\\","_")])
					print(name)
					out = cv.VideoWriter(name, fourcc, video_fps, (1280,720))
				elif found_portfolio == True:
					portfolio_frame_end = frame_counter
				...
				if found_portfolio == True:
					found_portfolio = False
					print(portfolio_frame_start, portfolio_frame_end)
					portfolio_frame_start = 0
					video_index+=1
			frame_counter+=1
		else:
