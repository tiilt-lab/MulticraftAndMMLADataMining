...
def get_point_and_time(fsacc, c_fixation_index, question_start, video_fps):
    c_fix_start_time, c_fix_end_time, c_fix_dur, c_x, c_y = fsacc[c_fixation_index]
    ...
    return c_fix_start_time, c_fix_end_time, c_fix_dur, c_x, c_y, c_fixation_index
...
for line in csv_file:
    ...
    fsacc = process_gaze_datapoints(gaze_file, video_fps, lambda w, x, y, z: fixation_detection(w, x, y))[1]
    ...
    # ------------------------------------------------------------------------------------------------------------
    while (cap.isOpened()):
        c_fix_start_time, c_fix_end_time, c_fix_dur, c_x, c_y, c_fixation_index = get_point_and_time(fsacc, c_fixation_index,
        ...
        if frame_counter >= question_start:
            ...
            if type(descriptors2) != None.__class__ and descriptors2.any():
                ...
                #                   quad_list)
with open('..\\results\\quadrants_fixations.json', 'w', encoding='utf-8') as f:
